...
import 'package:siakad_app/widget/jadwal/comp/edit_jadwal_pelajaran.dart';

class JadwalPelajaran extends StatefulWidget {
  const JadwalPelajaran(
      {Key? key, required this.className, required this.idClass})
      : super(key: key);
...
class _JadwalPelajaranState extends State<JadwalPelajaran> {
  ...
  Future<void> fungsiGetJadwal() async {
    String role = await getrole();
    setState(() {
      roleku = role;
    });
    ApiResponse response = await getJadwal(idClass: widget.idClass);
    if (response.error == null) {
      setState(() {
        print(widget.idClass);
        _JadwalList = response.data as List<dynamic>;
  ...
  Future<void> fungsiGetJadwalbyDay() async {
    ...
    ApiResponse response = await getJadwalbyDay(
        idClass: widget.idClass, idDay: int.parse(dropdownvalue));
    if (response.error == null) {
      setState(() {
        _JadwalList = response.data as List<dynamic>;
        _loading = _loading ? !_loading : _loading;
            ...
            child: _loading
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      JadwalModel jadwalModel = _JadwalList[index];
                      return Padding(
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      jadwalModel.firstName +
                                          ' ' +
                                          jadwalModel.lastName,
                                      style: const TextStyle(
                                        fontSize: 12,
                                        color: Color(0xff808DA6),
                                        fontFamily: 'Poppins',
