...
import 'package:siakad_app/widget/jadwal/comp/edit_jadwal_kerja.dart';

class JadwalKerja extends StatefulWidget {
  const JadwalKerja({Key? key}) : super(key: key);
...
class _JadwalKerjaState extends State<JadwalKerja> {
  ...
  Future<void> fungsiGetJadwalKerja() async {
    String role = await getrole();
    setState(() {
      roleku = role;
    });
    ApiResponse response = await getJadwalKerja();
  ...
  Future<void> fungsiGetJadwalKerjabyDay() async {
    showAlertDialog(context);
    ApiResponse response =
        await getJadwalKerjabyDay(id: int.parse(dropdownvalue));
    if (response.error == null) {
      setState(() {
        _JadwalKerjaList = response.data as List<dynamic>;
        _loading = _loading ? !_loading : _loading;
            ...
            child: _loading
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      JadwalKerjaModel jadwalKerjaModel =
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      jadwalKerjaModel.firstName +
                                          ' ' +
                                          jadwalKerjaModel.lastName,
                                      style: const TextStyle(
                                        fontSize: 12,
                                    ...
                                    Text(
                                      jadwalKerjaModel.startTime +
                                          ' : ' +
                                          jadwalKerjaModel.endTime,
                                      style: const TextStyle(
                                        fontSize: 12,
                                        color: Color(0xff808DA6),
                                        fontFamily: 'Poppins',
