...
class AbsenMonitoringEkstra extends StatefulWidget {
  const AbsenMonitoringEkstra({Key? key, required this.ekstrId})
      : super(key: key);

  final int ekstrId;

  @override
  State<AbsenMonitoringEkstra> createState() => _AbsenMonitoringEkstraState();
}

class _AbsenMonitoringEkstraState extends State<AbsenMonitoringEkstra> {
  ...
  MultipleEkstraModels multipleModels = MultipleEkstraModels(books: []);

  Future<void> fungsiGetSiswa() async {
    ApiResponse response = await getSiswaForAbsenEkstra(id: widget.ekstrId);
  ...
  void send(int idEkstra) async {
    showAlertDialog(context);
    String token = await getToken();
    var response = await http.post(
      Uri.parse(baseURL + '/api/monitoring/post/$idEkstra'),
      headers: {
                                                  ...
                                                  onChanged: (checked) {
                                                    setState(
                                                      () {
                                                        ...
                                                        if (isChecked1[index] ==
                                                            true) {
                                                          multipleModels.books.add(Books(
                                                              extracurricularId:
                                                                  widget
                                                                      .ekstrId,
                                                              studentId:
