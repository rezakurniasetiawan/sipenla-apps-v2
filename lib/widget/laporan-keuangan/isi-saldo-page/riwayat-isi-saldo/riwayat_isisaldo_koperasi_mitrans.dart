...
import 'detail_riwayat_isisaldo_mitrans.dart';

class RiwayatIsiSaldoKoperasiMitrans extends StatefulWidget {
  const RiwayatIsiSaldoKoperasiMitrans({Key? key}) : super(key: key);

  @override
  State<RiwayatIsiSaldoKoperasiMitrans> createState() =>
      _RiwayatIsiSaldoKoperasiMitransState();
}

class _RiwayatIsiSaldoKoperasiMitransState
    extends State<RiwayatIsiSaldoKoperasiMitrans> {
  ...
  Future<void> fungsigetHistoryUser() async {
    ApiResponse response =
        await getRiwayatIsiSaldoKoperasiMitrans(tanggal: tanggal.text);
            ...
            Expanded(
              child: ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    RiwayatIsiSaldoKoperasiModel riwayatIsiSaldoKoperasiModel =
                        riwayatIsiSaldoList[index];
                    String tanggal = DateFormat('dd-MM-yyyy')
                        .format(riwayatIsiSaldoKoperasiModel.createdAt);
                    String waktu = DateFormat('hh:mm:s')
                        .format(riwayatIsiSaldoKoperasiModel.createdAt);
                    return ItemListRiwayatSaldo(
                      colorStatus:
                          riwayatIsiSaldoKoperasiModel.status == 'pending'
                              ? const Color(0xffFFB711)
                              : riwayatIsiSaldoKoperasiModel.status == 'approve'
                      ...
                      balance: FormatCurrency.convertToIdr(
                          riwayatIsiSaldoKoperasiModel.grossAmount, 0),
                      code: riwayatIsiSaldoKoperasiModel.orderId,
                      waktu: tanggal,
                      ontaps: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => DetailRiwayatIsiSaldoMitrans(
                              code: riwayatIsiSaldoKoperasiModel.orderId,
                              tanggal: tanggal,
                              waktu: waktu,
                              saldo: FormatCurrency.convertToIdr(
                                  riwayatIsiSaldoKoperasiModel.grossAmount, 0),
                              status: riwayatIsiSaldoKoperasiModel.status ==
                                      'pending'
                                  ? 'Proses'
                                  : riwayatIsiSaldoKoperasiModel.status ==
                                          'approve'
                                      ? 'Berhasil'
                                      : 'Dibatalkan',
                              colors: riwayatIsiSaldoKoperasiModel.status ==
                                      'pending'
                                  ? const Color(0xffFFB711)
                                  : riwayatIsiSaldoKoperasiModel.status ==
                                          'approve'
