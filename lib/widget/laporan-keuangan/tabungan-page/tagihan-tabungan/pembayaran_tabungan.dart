...
import 'package:siakad_app/widget/laporan-keuangan/tabungan-page/tagihan-tabungan/sukses_bayar_tabungan.dart';

import '../../../../constan.dart';
import '../../../../services/auth_service.dart';
import 'package:http/http.dart' as http;

class PembayaranTabungan extends StatefulWidget {
  const PembayaranTabungan(
      {Key? key,
      required this.orderId,
      required this.valueTabungan,
      required this.vaNumber})
      : super(key: key);

  final String orderId, valueTabungan, vaNumber;

  @override
  State<PembayaranTabungan> createState() => _PembayaranTabunganState();
}

class _PembayaranTabunganState extends State<PembayaranTabungan> {
  Timer? timer;
  String status = '';
    ...
    if (response.statusCode == 200) {
      ...
      if (belum == '201') {
        ...
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => SuksesBayarTabungan(
  ...
  void initState() {
    super.initState();
    getData(orderId: widget.orderId);
    timer = Timer.periodic(const Duration(seconds: 3), (timer) {
      if (status == '201') {
        setState(() {
          getData(orderId: widget.orderId);
